...
int move(int keycode, void *arg)
{
	...
	if (keycode == KEY_D || keycode == KEY_A)
	{
		...
		if (game->map->map_grid[(int)game->player.pos.y][(int)(game->player.pos.x + move_dir_x * move_speed)] == '0')
			game->player.pos.x += move_dir_x * move_speed;
		if (game->map->map_grid[(int)(game->player.pos.y + move_dir_y * move_speed)][(int)game->player.pos.x] == '0')
