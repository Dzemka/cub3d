...
static void fill_buffer(t_game *game, int size_x, int size_y)
{
	...
	while (++y < size_y)
	{
		...
		while (++x < size_x)
		{
			if (((y == game->coord_start_hp.y || y == size_y - 1) && x >= game->coord_start_hp.x && x < size_x) || ((x == game->coord_start_hp.x || x == size_x - 1) && y >= game->coord_start_hp.y && y < size_y))
				game->health_frame_buffer[y][x] = 0xFF00FF;
			else if (y > game->coord_start_hp.y && y < size_y && x > game->coord_start_hp.x && x < size_x)
				game->health_frame_buffer[y][x] = 0x1;
			else
				game->health_frame_buffer[y][x] = 16777216;
		}
	}
	// exit(1);
}
...
void init_health_frame(t_game *game)
{
	...
	// printf("%d %d %f %f %d %d\n", size_x, size_y, game->coord_start_hp.x, game->coord_start_hp.y, width_frame, height_frame);
	// exit(1);
