...
void	death_action(t_game *game, int id, int i)
{
	...
	if (game->enemy[id]->action == game->enemy[id]->count_actions - 1)
		game->enemy[id]->action = game->enemy[id]->count_actions - 1;
	else if (game->enemy[id]->action < game->enemy[id]->count_actions - game->enemy[id]->count_dead - 1)
		game->enemy[id]->action = game->enemy[id]->count_actions - game->enemy[id]->count_dead;
	else if (game->enemy[id]->frame % (int)(ENEMY_DEATH_SPEED / (game->enemy[id]->count_dead + 1) + 1) == 0)
...
void spawn_action(int id, t_game *game)
{
	...
	if (game->enemy[id]->frame % (int)(ENEMY_SPAWN_SPEED / (game->enemy[id]->count_spawn + 1) + 1) == 0)
	{
		game->enemy[id]->action++;
		game->enemy[id]->frame = 0;
	}
...
void move_action(t_game *game, int id, int i)
{
	if (game->enemy[id]->frame % (100 / ENEMY_MOVE_SPEED) == 0)
		move(game, id, i);
	if (game->enemy[id]->frame % (ENEMY_WALK_SPEED / (game->enemy[id]->count_walk + 1) + 1) == 0)
		game->enemy[id]->action++;
...
void attack_action(t_game *game, int id, int i)
{
	...
	if (game->enemy[id]->frame % (int)(ENEMY_ATTACK_SPEED / (game->enemy[id]->count_attack + 1) + 1) == 0)
		game->enemy[id]->action++;
